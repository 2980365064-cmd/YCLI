...
from ycli.skill import SkillContextBuffer, SkillRegistry, SkillStateStore
from ycli.tools.base import ToolContext
from ycli.tools.builtins import load_skill
...
def test_skill_registry_layers_and_state(tmp_path, monkeypatch):
    ...
    _write_skill(project / ".ycli" / "skills", "project-only", "project desc", "v2")
...
def test_load_skill_pushes_body_into_context_buffer(tmp_path, monkeypatch):
    monkeypatch.setenv("HOME", str(tmp_path / "home"))
    _write_skill(tmp_path / ".ycli" / "skills", "demo", "demo desc", "v1", body="demo body")
