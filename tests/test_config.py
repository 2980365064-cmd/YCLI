...
def test_config_precedence(tmp_path, monkeypatch):
    ...
    (home / ".ycli").mkdir(parents=True)
    (project / ".ycli").mkdir(parents=True)
    (home / ".ycli" / "config.json").write_text(
        json.dumps({"llm": {"provider": "home", "model": "home-model"}}),
        encoding="utf-8",
    )
    (project / ".ycli" / "config.json").write_text(
        json.dumps({"llm": {"provider": "project", "model": "project-model"}}),
        encoding="utf-8",
    )
    (project / ".env").write_text("YCLI_MODEL=env-file-model\n", encoding="utf-8")
    monkeypatch.setenv("HOME", str(home))
    monkeypatch.setenv("YCLI_PROVIDER", "process")
...
def test_provider_specific_api_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("YCLI_PROVIDER", "deepseek")
    monkeypatch.delenv("YCLI_API_KEY", raising=False)
