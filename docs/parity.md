...
- Memory:
  ...
  - project memory files `YAI.md`, `.ycli/YAI.md`, local variants
- Skills:
  - built-in/user/project skill layers
  - user/project `.ycli/skills/*/SKILL.md`
  - `~/.ycli/skills.json` disabled-state store
...
- MCP:
  ...
  - `ycli mcp init-chrome`
  - `ycli mcp list`
  - YCLI MCP server over stdio/http for built-in tools
- Chrome DevTools MCP:
...
uv run ycli mcp serve --transport http --port 3999
```
