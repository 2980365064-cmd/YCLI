"""YCLI 交互式 REPL — 基于 prompt-toolkit 的终端交互循环。

功能：
- 富文本工具栏（模型、token 用量、HITL 模式）
- 22 个 slash 命令（/help, /config, /memory, /model 等）
- 人工审批对话框
- 会话历史持久化
"""
...
from ycli.agent import Agent, AgentOrchestrator, PlanExecuteAgent
from ycli.bootstrap import build_tool_registry
from ycli.config import YcliConfig, config_to_public_dict
...
from ycli.rag import CodeIndex
from ycli.render import RichRenderer
from ycli.runtime import DurableTaskManager
from ycli.skill import SkillRegistry
from ycli.snapshot import SnapshotService
from ycli.tools import ToolRegistry
...
async def start_repl(cwd: str, config: YcliConfig) -> None:
    console = Console()
    ...
    history_path = Path.home() / ".ycli" / "history" / "prompt_history.txt"
...
async def _handle_slash(
    ...
    if command == "/help":
        ...
        table = Table(title="YCLI Context")
        ...
        console.print("Use `ycli mcp serve --transport stdio|http --port 3000` to expose tools.")
    elif command == "/task":
...
async def _memory_command(arg: str, console: Console, cwd: str, config: YcliConfig) -> None:
...
def _hitl_command(arg: str, console: Console, config: YcliConfig) -> None:
...
def _model_command(arg: str, console: Console, config: YcliConfig) -> None:
    if not arg:
...
def _task_command(arg: str, console: Console) -> None:
    manager = DurableTaskManager(Path.home() / ".ycli" / "tasks" / "tasks.db")
    sub, _, rest = arg.partition(" ")
    if sub == "add" and rest:
...
def _approval_prompt(request: dict[str, Any], console: Console, config: YcliConfig) -> str:
