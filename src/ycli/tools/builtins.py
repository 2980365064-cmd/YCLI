"""内置工具集 — YCLI 预置的 14+ 个本地工具。

涵盖：
- 文件操作：read_file, write_file, list_directory
- 搜索：glob_files, grep_code, search_code
...
from ycli.lsp import diagnose_file
from ycli.memory import MemoryManager
from ycli.policy import CommandGuard, PathGuard
from ycli.rag import CodeIndex
from ycli.skill import SkillRegistry
from ycli.snapshot import SnapshotService
from ycli.tools.base import Tool, ToolContext, ToolResult, object_schema
from ycli.web import fetch_url, search_web
...
def get_builtin_tools() -> list[Tool]:
    tools = [
        ...
        Tool(
            name="load_skill",
            description="Load a named YCLI skill manual from user/project skill directories.",
            parameters=object_schema(
        ...
        Tool(
            name="revert_turn",
            description="Restore the workspace to a previous YCLI side-history snapshot.",
            parameters=object_schema(
