"""MCP 服务端 — 将 YCLI 的内置工具暴露为 MCP 协议。

支持两种传输方式：
- stdio: 基于行分隔的 JSON-RPC（适合本地进程通信）
- HTTP: 基于 ThreadingHTTPServer（适合远程访问）
"""
...
async def _handle_request(request: dict[str, Any], cwd: str) -> dict[str, Any]:
    ...
    if method in {"initialize", "notifications/initialized"}:
        return {"jsonrpc": "2.0", "id": request_id, "result": {"serverInfo": {"name": "ycli"}}}
...
def serve_http(port: int = 3000, cwd: str | None = None) -> None:
    ...
    print(f"YCLI MCP server listening on http://127.0.0.1:{port}", flush=True)
