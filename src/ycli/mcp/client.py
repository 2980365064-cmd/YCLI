"""MCP 客户端管理器 — 连接 stdio/HTTP MCP 服务器并注册远程工具。

McpClientManager 负责：
1. 从 mcp.json 加载服务器配置
2. 通过 stdio 或 HTTP 连接 MCP 服务器
3. 将远程工具包装为本地 Tool 对象（命名格式 mcp__server__tool）
4. 创建虚拟的 resource/prompt 工具
"""
...
from ycli.mcp.config import McpServerSpec, load_mcp_server_specs
