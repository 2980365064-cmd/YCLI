"""Runtime API 服务器 — 提供 HTTP 接口管理 Agent 会话和后台任务。

主要功能：
- 会话管理（创建/列出/查询线程和 turn）
...
from ycli.runtime.tasks import DurableTaskManager
...
class RuntimeApiServer:
    def __init__(
        ...
        *,
        ...
        self.port = port
        self.db_path = Path.home() / ".ycli" / "runtime" / "runtime.db"
        self.db_path.parent.mkdir(parents=True, exist_ok=True)
        self.task_manager = DurableTaskManager(Path.home() / ".ycli" / "tasks" / "tasks.db")
    ...
    def serve_forever(self) -> None:
        ...
        print(f"YCLI Runtime API listening on http://127.0.0.1:{self.port}", flush=True)
        try:
    ...
    def _ensure_llm_key(self) -> None:
        if not self.config.llm.api_key:
            raise ValueError(
                "YCLI_API_KEY is not configured. Runtime turns/tasks need a working LLM key."
            )
...
def runtime_api_key(explicit: str | None = None) -> str:
    key = explicit or os.environ.get("YCLI_RUNTIME_API_KEY")
    if not key:
        raise ValueError("YCLI_RUNTIME_API_KEY is required for Runtime API")
    return key
